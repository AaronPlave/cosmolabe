...
Catalog field names mirror Cosmographia's `Sensor` schema (`UniverseLoader::loadSensorGeometry()`) so authored catalogs port without churn; extensions are additive. Mind the qualifier: what loads is the *upstream* `type: "Sensor"` block. The SPICE-enhanced sensor catalogs the NAIF user's guide documents are `class: "sensor"` with `geometry.type: "Spice"` and `instrName`, and those draw no instrument at all today — see §1.2 of [docs/cosmographia-parity.md](docs/cosmographia-parity.md).

- **Sensor footprints painted on the target surface.** Compute the FOV–body intersection (cone edges raycast against the target ellipsoid; for tiled terrain, against the active LOD heightfield) and render the resulting polygon as a decal on the body surface. Drives the NASA Psyche / Mars-flyby use case. Base schema (Cosmographia-compatible, `type: "Sensor"`): `target`, `range`, `horizontalFov` / `verticalFov` (degrees, full apex — *not* half-angle), `shape` (`"elliptical"` | `"rectangular"`), `frustumColor`, `frustumOpacity`, `orientation` quaternion `[w,x,y,z]`. Footprint = line-strip polygon on the surface, scaled inward slightly to avoid Z-fighting; `footprintOpacity` and `gridOpacity` are both Cosmographia sensor-catalog parameters, not hardcoded — but note that its sensor `footprintOpacity` styles the *outline at the far end of the frustum*, not a mark on the surface (`sideDivisions` defaults to 125). Marks on the surface are a different item entirely: read §1.3 of [docs/cosmographia-parity.md](docs/cosmographia-parity.md) before naming the extensions below. A Cosmographia *sensor* is always-on while rendered, but the program pairs it with a separate `observation` item (`class: "observation"`, `geometry.type: "Observations"`) that names the sensor and carries the actual observation windows as `groups: [{ startTime, endTime, obsRate }]`, painted in the target's body-fixed frame; the sensor's `onlyVisibleDuringObs` ties the live frustum to those same windows. Prefer Cosmographia's field names — and its split into a second item — unless there is a reason not to:
  - `active: [{ start, end }]` observation-window list — footprint only paints inside these windows; replays deterministically from the catalog. This is Cosmographia's `groups`, minus its `obsRate` (seconds between footprints).
  - `accumulate: true` — stamps written into a persistent overlay texture on the target body so a flyby leaves a painted swath. Cosmographia's equivalent is `obsRate: 0` on an observation group, which draws a continuous swath rather than discrete footprints; its `fillInObservations` is unrelated — that only chooses filled versus outlined footprints. Off by default.
...
- Track and close any remaining gaps versus Cosmographia's full schema — the current inventory is [docs/cosmographia-parity.md](docs/cosmographia-parity.md), which reads the Cosmographia user's guide against our loader. Live gaps from it: the object form of `trajectoryFrame` (`{ "type": "BodyFixed", "body": "Cassini" }` — what every stock sensor and observation uses) is misread as ecliptic; a SPICE-enhanced sensor block (`class: "sensor"`, `geometry.type: "Spice"`, `instrName`) draws no frustum at all; `m` in a duration means minutes to Cosmographia and metres to our shared unit parser, so `"90 m"` becomes 0.09 seconds; the guide's own implicit `arcs` boundaries are misdated, giving the second arc a one-year window; an item's `startTime`/`endTime` lifetime is parsed and dropped; and `trajectoryPlot.lineWidth`, `label.fadeSize` and `ArcSpec.bodyFrame` are declared but read by nobody.

  ...
  {#if !loading && !uiHidden}
    ...
    {#if pickResult}
      <div class="absolute top-3 right-3 z-20 bg-black/90 backdrop-blur-md border border-border rounded-lg p-2.5 min-w-50 text-[12px]">
        ...
        <div class="flex justify-between gap-4 leading-relaxed"><span class="text-text-secondary">Sampled alt</span><span class="font-mono text-text-primary">{pickResult.altKm >= 0 ? '+' : ''}{fmtCoord(pickResult.altKm * 1000, 1)} m</span></div>

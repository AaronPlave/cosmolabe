<script lang="ts">
  import {
    ...
    zoomScrubber, panScrubber, resetScrubberZoom,
    etToShortDate,
  } from '../lib/viewer-state.svelte';
  import { formatDuration } from '../lib/scrubber-math';
  import { clampFraction } from '../lib/scrubber-math';
  ...
  // Minimap viewport fractions (0-1 of base range)
  ...
  let startLabel = $derived(etToShortDate(vs.scrubBaseMin));
  let endLabel = $derived(etToShortDate(vs.scrubBaseMax));
  ...
  let panningMinimap = $state(false);
  ...
  function onMinimapPointerDown(e: PointerEvent) {
    ...
    panScrubber(clampFraction((e.clientX - rect.left) / rect.width));
  }

  function onMinimapPointerMove(e: PointerEvent) {
    if (!panningMinimap || !minimapEl) return;
    const rect = minimapEl.getBoundingClientRect();
    panScrubber(clampFraction((e.clientX - rect.left) / rect.width));
  }

  function onMinimapPointerUp(e: PointerEvent) {
    if (!panningMinimap) return;
    minimapEl?.releasePointerCapture(e.pointerId);
    panningMinimap = false;
  }
...
<div class="absolute bottom-3 left-3 right-3 z-20 flex flex-col gap-1.5">
  <!-- Floating minimap (visible when zoomed) -->
  {#if isZoomed}
    <div class="border border-border rounded-lg bg-black/90 backdrop-blur-md px-3 py-1.5 flex items-center gap-2">
      <span class="font-mono text-[10px] text-text-muted shrink-0">{startLabel}</span>
      ...
      <span class="font-mono text-[10px] text-text-muted shrink-0">{endLabel}</span>
      <span class="font-mono text-[10px] text-accent shrink-0">{rangeLabel}</span>
      <button class="icon-btn" onclick={resetScrubberZoom} title="Reset zoom (double-click scrubber)">
        <X size={12} />
      </button>
    </div>
  {/if}
  ...
  <!-- Main bottom bar -->
  <div class="border border-border rounded-lg bg-black/90 backdrop-blur-md flex flex-col gap-0.5 px-3 py-1.5">
    {#if showShortcuts}
    ...
    <div class="flex items-center gap-1.5 w-full">
      ...
      <button class="icon-btn" class:text-accent={measureActive} onclick={onToggleMeasure} title="Geometry"><Ruler size={15} /></button>
